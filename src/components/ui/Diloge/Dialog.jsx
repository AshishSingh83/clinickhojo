...
import { AiOutlineClose } from 'react-icons/ai';
import { FaSpinner } from 'react-icons/fa'; // Using FontAwesome spinner for illustration
      ...
      >
        ...
        <div className="flex items-center">
          <button
            ...
            onClick={() => onDialog(true)}
            disabled={sniper} // Disable button when sniper is true
          >
            {sniper ? <FaSpinner className="animate-spin" /> : "Yes"} {/* Conditionally render spinner */}
          </button>
          <button
            className={`bg-green-500 text-white px-6 py-2 rounded ${sniper ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
            onClick={() => onDialog(false)}
            disabled={sniper} // Disable button when sniper is true
          >
            No
